...
void ResumM (const string & PathToFolderRD, const ParamsP11 & params, const bool & ExportM, StoreM * TableM) {
	...
	for (unsigned int order = 0 ; order < 1 ; order++) { // CHANGE 2 TO 1 HERE FROM ORIGINAL REDSHIFTBIASEFT_C

		size_t Nlp ;
		if (order == 0) Nlp = 3 ; // CHANGE 5 TO 3 HERE FROM ORIGINAL REDSHIFTBIASEFT_C
		if (order == 1) Nlp = 3 ;
